...
import io.toylanguage.gui.models.LockModel;
...
public class MainScreen implements InvalidationListener {
    ...
    public TableView<LockModel> lockTableView;
    public TableColumn<LockModel, Integer> lockTableKey;
    public TableColumn<LockModel, Integer> lockTableValue;
    ...
    private void initializeCellValueFactories() {
        ...
        lockTableKey.setCellValueFactory(new PropertyValueFactory<>("lockLocation"));
        lockTableValue.setCellValueFactory(new PropertyValueFactory<>("lockValue"));
    }
    ...
    private void refreshAllViews(List<ProgramState> programStates) {
        ...
        refreshLockTableView(selectedProgramState);
...
//        executionStackView.getItems().add(programState.getExecutionStack().toString());
    ...
    private void refreshLockTableView(ProgramState programState) {
        lockTableView.getItems().clear();
        lockTableView.setItems(FXCollections.observableArrayList(
                programState.getLockTable().stream()
                        .map(lockEntry -> new LockModel(lockEntry.getKey(), lockEntry.getValue()))
                        .collect(Collectors.toList())
        ));
    }
