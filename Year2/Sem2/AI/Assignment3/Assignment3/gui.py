...
def movingDrone(currentMap, path, speed = 4,  markSeen = True):
...
def displayMap(config):
    screen = initPyGame((config["map-size"] * 20, config["map-size"] * 20))
    screen.blit(image(config["map"]), (0, 0))
    pygame.display.flip()
    closePyGame()